...
Поэтому надо исследовать вопрос, что как меняются вещественная и мнимая части вектора при комплексном повороте.
В случае колебательной составляющей, при умножении комплексной экспоненты (комплексной э-м гармоники ранга 1) на $e^{2\I\pi t}$ меняется только ее фаза, это не влияет ни на норму, ни на частоту вещественной или мнимой части экспоненты, от которых зависят рассматриваемые меры.
В случае комплексной э-м гармоники ранга 2 (она будет идентифицироваться только в методе по регулярности углов) в методе учитывается эта особенность сингулярных векторов и подбирается нужный поворот. 
В случае тренда 
при умножении какого-то комплексного вектора на  $e^{2\I\pi t}$ может поменяться норма вещественной и мнимой частей, поэтому возможно наиболее устойчивый вариант --- применять метод к восстановленным рядам, а не к сингулярным векторам.
...
\STATE Получаем группу индексов $I^{(\mathbb{P})}$ по $\{V_j\}_{j \in I_1^{(\mathbb{P})} \cup I_2^{(\mathbb{P})}}$ и $\rho_0$ с помощью \eqref{eq:pgram_I_p_mssa}.
\end{algorithmic}
\end{algorithm}
...
К правым сингулярным векторам метод можно применить аналогичном способом, как для частотного метода. 
...
%
%\begin{gather*}
%\tau_{j,j+1} :=  \min_p \{ \max_{0 < k \leqslant {K_p}/2}{\left(\rho_{\{\widehat{V}_i^{(p)}, \widehat{V}_{j}^{(p)}\}}(k/{K_p}) + \rho_{\{\{\widehat{V}_i^{(p)}, \widehat{V}_{j}^{(p)}\}}((k+1)/{K_p})\right)}, p=1,\ldots, s
% \},
%\end{gather*}

\begin{algorithm}[!hhh]
\caption{MSSA. Метод по регулярности углов для колебательной составляющей: вариант с правыми сингулярными векторами}
...
Алгоритм \ref{alg:cssa_pgram} также, как и алгоритм \ref{alg:cssatau_2} можно обобщить на случай, когда число э-м гармоник, которые хотим выделить, известно. Просто сортируем значения $\rho_i$ из формулы \eqref{eq:rho_i_cssa} по убыванию и берем первые $m$ компонент. Результаты применения с таким алгоритмом представлены на рис.~\ref{fig:a_many_after3}.
...
Применим методы автоматической группировки, во фрагменте \ref{frag:cssa_trend} применяем метод низких частот для тренда, используя созданную функцию general.grouping.auto. Используем значение 0.01 для частоты и 0.9 для порога, функция выделила только одну первую компоненту. 
...
%две пары индексов: d1\_idx --- индексы, относящиеся к э-м гаромнике ранга $d=1$, и d2\_idx --- индексы, относящиеся к э-м гармонике ранга $d=2$; plotd1 --- специально созданная функция, код которой можно найти в \cite{polina_zhornikova_2018_1252476}.
