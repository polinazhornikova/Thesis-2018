...
Пусть $P = (p_1,\ldots,p_L)^{\textrm{T}}$ и $Q=(q_1,\ldots,q_L)^{\textrm{T}}$ --- два вещественных вектора длины $L$. Введем для них меру
\begin{gather} \label{eq:tau1}
\tau(P, Q) := \hat{\D}(\Theta) =\frac{1}{L-1}\sum_{k=1}^{L-1}{\left(\theta_k  - \bar{\theta}\right)^2},
\end{gather}
где $\Theta=(\theta_1,\ldots,\theta_L)^{\textrm{T}}$,
$\bar{\theta} = \sum_{k=1}^{L-1}{\theta_k}/(L-1)$, $\theta_k$ --- угол между
$\left(p_k, q_{k}\right)^{\textrm{T}}$ и $\left(p_{k+1}, q_{k+1}\right)^{\textrm{T}}$, т.е.
\begin{gather*}
...
%\begin{enumerate}
%\item
$L$-Ранг $d$ ряда $\mathbb{S}$ равен $1$, если $\omega = 0.5$, в остальных случаях $d=2$.
%\item Если $d=2$, то подпространство $\Lambda^{(1)}$ имеет базис \\
%\begin{gather*}
%\{(1, e\cos(2\pi\omega),\ldots,e^{\alpha (L-1)}\cos(2\pi (L-1) \omega))^\textrm{T}, \\
%(0, e\sin(2\pi\omega),\ldots,e^{\alpha (L-1)}\sin(2\pi (L-1) \omega))^\textrm{T}\}.
%\end{gather*}
%\item Если $d=2$, то подпространство $\Lambda^{(2)}$ имеет базис \\
%\begin{gather*}
%\{(с^{(1)}_1,\ldots,c_K^{(1)}, с^{(2)}_1,\ldots,c_K^{(2)})^\textrm{T},\ldots,
%(d^{(1)}_1,\ldots,d_K^{(1)}, d^{(2)}_1,\ldots,d_K^{(2)})^\textrm{T}\},
%\end{gather*}
%где
%\begin{gather*}
%c^{(1)}_j =
...
Пусть $\alpha = 0$, $\omega \not = 0.5$ и $L\omega$  и $K\omega$ --- целые, $s=2$. Тогда собственные числа сингулярного разложения для двумерного ряда ряда $\mathbb{S}$ одинаковы и имеют вид $\lambda_1=\lambda_2=(a^2 + b^2)LK/4$.
\end{Th}
...
Итоговым результатом периодограммного метода, реализуемого алгоритмом \ref{alg:1d_pgram}, являются индексы
...
Для вещественных векторов $P = (p_1,\ldots,p_L)^{\textrm{T}}$ и $Q=(q_1,\ldots,q_L)^{\textrm{T}}$ длины $L$ определим
\begin{gather*}
\tau(P, Q) := \hat{\D}(\Theta) =\frac{1}{L-1}\sum_{k=1}^{L-1}{\left(\theta_k  - \bar{\theta}\right)^2},
\end{gather*}
где $\Theta=(\theta_1,\ldots,\theta_L)^{\textrm{T}}$,
...
помощью порога, т.е. к э-м гармонике относить те вектора $U_1$ и $U_2$, значение $\tau(U_1, U_2)$ для которых меньше заданного порога, но в работе не было дано никаких обоснований для выбора порога.
...
Приведем алгоритм \ref{alg:1dtau} метода, описанный в \cite{Zhornikova2016}.
...
В данной главе приведем обобщения некоторых методов из глав \ref{sec:1d_methods} и \ref{sec:tau1} для других типов объекта $\mathbb{X}$ и вариантов метода SSA.
...
Затем отдельно для каждого из вариантов метода SSA: CSSA, MSSA, 2D-SSA), будет приведено обобщение частотного метода идентификации из раздела \ref{sec:1d_freq_method}, периодограммного метода для колебательной компоненты из раздела \ref{sec:1d_pgram_method} и метода идентификации колебательной компоненты по регулярности углов из главы \ref{sec:tau1}.
...
Комплексные временные ряды это искусственная вещь, в реальных задачах такое почти не встречается, за исключением комплексной экспоненты, т.е. э-м гармоники ранга 1. Подобный пример будет приведен далее в разделе \ref{sec:use_cssa}.
Поэтому при идентификации комплексной колебательной составляющей наша главная цель --- идентифицировать гармоники ранга 1, но если при этом будет возможно идентифицировать гармоники ранга 2, то это будет отмечено.
...
В CSSA и левые и правые сингулярные вектора и восстановленные ряды являются комплексными, значит, к ним можно применять алгоритм равнозначно.

Предлагается алгоритм \ref{alg:freqcssa}, похожий на частотный алгоритм \ref{alg:freq1d} для 1D-SSA, но в котором в отличие от алгоритма \ref{alg:freq1d} на 1 шаге будем вычислять меру отдельно для мнимой и вещественной частей $\mathbb{Y}_i$ и брать из них максимальную для использования на 2 шаге алгоритма.
...
Согласно разделу \ref{sec:cssa_theory} главы \ref{sec:ssa_theory} элементы комплекснозначного э.-м. гармонического ряда $\mathbb{S}$ имеют вид
\begin{equation*}
 s(n) = e^{\alpha n} (a\cos(2\pi\omega n + \phi_1) + \I \,b\cos(2\pi\omega n + \phi_2)),
\end{equation*}
где $0 \leqslant\phi_1, \phi_2 < 2\pi$, $a, b \not = 0$, $0<\omega \leqslant 0.5$, $\sin \phi_1 \not = 0$ или $\sin \phi_2 \not = 0$ для $\omega = 0.5$.

Из утверждения \ref{th:cssa_ex_mod_im_vec} известно, для комплекснозначного э.-м. ряда ранг ряда $d$ может равняться или $2$, или $1$, т.е. ему может соответствовать либо один, либо два левых сингулярных вектора.
...
Как уже было сказано в разделе \ref{sec:cssa_theory} из утверждения \ref{th:cssa_ex_mod_im_vec} этого же раздела следует, что если $\omega$ и $\phi_j$ такие, что $d = 2$, то элементы сингулярных векторов $U_1$ и $U_2$ ряда $\mathbb{S}$ можно представить в  виде \eqref{eq:uuu1} и \eqref{eq:uuu2} соответственно.
Мнимая и вещественная части векторов имеют одинаковую частоту. Из утверждения \ref{th:cssa_ex_mod_im_vec} также очевидно следует, что в случае $d=1$ мнимая и вещественная часть тоже имеют одинаковую частоту.
...
В данном случае подход для случаев $\omega=0.5$ и $\omega \not =0.5$ будет одинаковым.
...
\end{gather}
Ниже приведем алгоритм \ref{alg:cssa_pgram} периодограммного метода для CSSA.
\begin{algorithm}[!hhh]
...
\label{sec:tau_cssa}
Описание метода из главы \ref{sec:tau1} для комплексных э-м гармоник, основанного на специальной мере $\tau$, задаваемой формулой \eqref{eq:tau1}, было приведено в бакалаврской работе \cite{Zhornikova2016}. Опишем метод и приведем этот алгоритм.
...
Для $d=1$ единственный сингулярный вектор имеет вид \eqref{eq:vid_d1} и, согласно  утверждению \ref{th:th_tau_cssa_d1} раздела \ref{sec:cssa_theory} рассматриваем значения $\tau(\RE U_1, \IM U_1)$.
Отбираем индексы $i$ векторов $U_i$, у которых значение меры $\tau(\RE U_1, \IM U_1)$
меньше заданного порога $t_0$. Алгоритм \ref{alg:cssatau} приведен ниже.
...
Обобщим теперь алгоритм \ref{alg:freq1d} идентификации тренда из раздела \ref{sec:1d_freq_method} на случай многомерного ряда. Алгоритм \ref{alg:freq1d} можно было применять либо к левым или правым сингулярным векторам, либо к восстановленным рядам.

Из утверждения \ref{th:mssa_vec} мы знаем, что в MSSA левые сингулярные вектора являются одномерными вещественнозначными временными рядами длины $L$ и состоят из элементов одного из рядов $\mathbb{X}^{(1)}, \ldots, \mathbb{X}^{(s)}$. Т.е. левые сингулярные вектора имеют такой же вид, как в случае 1D-SSA, к ним можно применить частотный алгоритм \ref{alg:freq1d} точно так же.

Введем обозначения для правого сингулярного вектора ${V}_{i}$: ${V}_{i} = ({V}_i^{(1)}, \ldots,{V}_i^{(s)})$, $i \in I$,
где $V_i^{(p)}$, $p=1,\ldots,s$ --- вектор с $K$ элементами вектора ${V}_{i}$, начиная с элемента номер $K(p-1)+ 1$ и заканчивая элементом номер $pK$.

Правые сингулярные вектора также являются одномерными вещественнозначными временными рядами длины $L$ и состоят из последовательных частей длины $K$ каждого из рядов $\mathbb{X}^{(1)}, \ldots, \mathbb{X}^{(s)}$. Для правых сингулярных векторов частотный алгоритм применяем так:
на первом шаге делим вектор на $s$ частей ${V}_i^{(1)}, \ldots,{V}_i^{(s)}$, для каждой считаем значение меры по формуле \eqref{eq:T_measure} и берем максимальное значение этой меры для использования на втором шаге.

Восстановленные ряды в случае MSSA являются также многомерными временными рядами, к ним применяем алгоритм так: на первом шаге для каждого из $s$ рядов считаем меру по формуле \eqref{eq:T_measure} и берем максимальное значение этой меры для использования на втором шаге.

Предлагается использовать алгоритм \ref{alg:freqmssa_1} для случая, когда применяем метод к левым сингулярным векторам, алгоритм \ref{alg:freqmssa_2} для случая, когда применяем метод к правым сингулярным векторам и алгоритм \ref{alg:freqmssa_3} для случая, когда применяем метод к восстановленным рядам.
...
\ENSURE Группа индексов компонент $J \subset I$.
\STATE  Для каждого вектора $U_i$, $i \in I$, вычисляем значение $T(U_i; \omega_1, \omega_2)$ с помощью  \eqref{eq:T_measure}.
\STATE $J$ --- группа индексов $i \in I$, таких что $T(U_i; \omega_1, \omega_2) \geqslant T_0$
\end{algorithmic}
\end{algorithm}
...
рядов $\mathbb{X}^{(1)}, \ldots, \mathbb{X}^{(s)}$. Поэтому к ним парный частотный метод можно применить точно так же, как в случае 1D-SSA. Ниже приведем алгоритм \ref{alg:mssa_pgram_1} периодограммного метода для MSSA и левых сингулярных векторов.
...
Итоговым результатом периодограммного метода, реализуемого алгоритмом \ref{alg:mssa_pgram_2}, являются индексы
\begin{gather} \label{eq:pgram_I_p_mssa}
I^{(\mathbb{P})} = \{ (i,j): \rho_{i,j} \geqslant\rho_0 \} \cup \{ i: \rho_{i} \geqslant\rho_0 \}.
\end{gather}

Ниже приведем алгоритм \ref{alg:mssa_pgram_2} периодограммного метода для MSSA и правых сингулярных векторов.
...
%\item Рассмотрен другой парный частотный метод автоматической идентификации, предложенный в \cite{Alexandrov2006} и \cite{Shmelov2011}. Метод реализован на языке R. Для меры этого метода проведены аналогичные исследования, как в пунктах 1 и 2, на тех же рядах. Проведено сравнение процента пересечения распределений мер, соответствующих сигналу и шуму, для периодограммного метода и метода, основанного на $\tau_1$. Последний алгоритм дал лучшие результаты.
...
Обозначаем значение порога для метода с $\tau$ через $t_0$, а для периодограммного метода через $\rho_0$.
