...
\begin{remark}
На вход алгоритма могут подаваться не только первые $r$ сингулярных векторов. Может подавать любое множество последовательных векторов (множество, где индекс каждого следующего вектора на один больше предыдущего) из множества $\{U_j\}_{j=1}^{d}$. В алгоритм для простоты изложения был приведен вариант с первыми $r$ векторами.
\end{remark}
...
Приведем примеры работы всех алгоритмов на простых модельных примерах.

\subsection{1D-SSA}
Задаем одномерный вещественный ряд с экспонентой в качестве тренда, э-м гармоникой в качестве колебательной составляющей, и белым шумом.
...
\begin{fragment}[1D-SSA: инициализация ряда]
\label{frag:1dssa_series}
\input{1dssa_series.tex}
\end{fragment}
\bfgh
        \begin{center}
{\includegraphics[width=4.5in]{1dssa_series}}%
        \end{center}
        \caption{1D-SSA. Пример ряда}
        \label{fig:1dssa_series}
\efg
