...
library(Rssa)
s <- ssa(x, L = 100)
fragmentSkip(pdf("img_ch1/1dssa_vectors.pdf", paper = "special", width = 4, height = 3))
plot(s, type="vectors", layout = c(5, 2))
fragmentSkip(dev.off())
fragmentSkip(pdf("img_ch1/1dssa_vectors_pair.pdf", paper = "special", width = 4, height = 3))
...
r <- reconstruct(s, groups = list(T = g_trend, P = c(g_em_freq$I_1, g_em_freq$I_2)))
