...
for (dir in c('age_4', 'age_8')){
  for (gene in c('Kr','gt')){
    ...
    for (file in files){
      ...
      rec <- reconstruct(bss, groups = list(1:r))
      
      print(plot(residuals(bss, 1:r), type = "nuclei-section",
           coord = "x", at = 50, tol = 5,
           ref = TRUE, col = "blue", main=file))
      print(plot(residuals(bss, 1:r), type = "nuclei-section",
           coord = "y", at = 50, tol = 5,
           ref = TRUE, col = "blue", main=file))
      print(plot(noise.model(bss, groups = 1:r, model = "mult"), main=file))
      ...
      noise <- noise.model(bss, 1:r, averaging.type = "none", offset = 0)
    ...
    setwd(cur_dir)
    write.table(results2D_real, paste0("realdata_results/2D_", dir, "_",gene, ".txt"))
    results.data <- data.frame(coef_y = sqrt(sigma.mult3))
    row.names(results.data) <- all_embyo
    write.table(results.data, paste0("realdata_results/data/2D_", dir, "_",gene, ".txt"))
    ...
    write.table(results.data.prop, paste0("realdata_results/data/prop_2D_", dir, "_",gene, ".txt"))
